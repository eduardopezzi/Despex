import {Controller, Get, Logger, Param, ParseIntPipe, Post, UploadedFile, UseInterceptors} from '@nestjs/common';
...
export class InvoicesController {
    ...
    findOne(@Param(RouteParam.Id, ParseIntPipe) id: number): Promise<InvoiceEntity> {
    ...
    async upload(@UploadedFile() file: Express.Multer.File): Promise<{ id: number; message: string }> {
        this.logger.log(`Received upload: ${file.originalname}`);
        const invoice = await this.invoicesService.upload(file.filename, file.originalname);
